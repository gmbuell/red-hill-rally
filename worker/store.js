...
const CIRCLE_PRIORITIES = new Set(['people', 'arts']); // $2,500 named tiers

export async function campaignStats(db, { CAMPAIGN }) {
  const [totals, byPriority, byClassroom, roll] = await db.batch([
  ...
  const donors = roll.results.map((row) => {
    ...
    return {
      ...
      anon: !isPublic,
      circle: row.amount_cents >= 250000 && CIRCLE_PRIORITIES.has(row.priority),
    };
  });
